<script lang="ts">
	...
	import type { ReportData, ReportTabId } from '$lib/types/report';

	let { reportData }: { reportData: ReportData | null } = $props();
	...
	const reportText = $derived(
		reportData
			? [
					...
					`Revenue Forecast: ${reportData.revenueForecast}`,
					`Optimal Price Range: ${reportData.optimalPriceRange}`,
					`Current Average Price: ${reportData.currentAveragePrice}`,
					'',
					'Top Positive Feedback',
					...reportData.positiveFeedback.map((item) => `- ${item.label}`),
					'',
					'Top Customer Objections',
					...reportData.objections.map((item) => `- ${item.label}`)
				].join('\n')
			: ''
	);
...
{#if reportData}
	<Card class="overflow-hidden">
		<ReportHeader
			filename={`simulation-report-${reportData.id}.txt`}
...
{:else}
	<EmptyState
		description="A completed simulation report is not available for your account yet."
		title="No accessible report"
	/>
{/if}
